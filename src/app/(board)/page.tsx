              ...
              <Card.Root href={`/issues/${issue.id}`} key={issue.id}>
                ...
                <Card.Footer>
                  <Button>
                    ...
                    <span className="text-sm">{issue.issueNumber}</span>
                  </Button>
      ...
      <Section.Root>
        ...
        <Section.Content>
          {issues.todo.length === 0 ? (
            <div className="flex items-center justify-center py-8 text-center">
              <p className="text-sm text-navy-300">
                No to-dos matching your filters
              </p>
            </div>
          ) : (
            issues.todo.map((issue) => (
              <Card.Root href={`/issues/${issue.id}`} key={issue.id}>
                <Card.Header>
                  <Card.Number>TODO-{issue.issueNumber}</Card.Number>
                  <Card.Title>{issue.title}</Card.Title>
                </Card.Header>
                <Card.Footer>
                  <Button>
      ...
      <Section.Root>
        ...
        <Section.Content>
          {issues.in_progress.length === 0 ? (
            <div className="flex items-center justify-center py-8 text-center">
              <p className="text-sm text-navy-300">
                No issues in progress matching your filters
              </p>
            </div>
          ) : (
            issues.in_progress.map((issue) => (
              <Card.Root href={`/issues/${issue.id}`} key={issue.id}>
                <Card.Header>
                  <Card.Number>INP-{issue.issueNumber}</Card.Number>
                  <Card.Title>{issue.title}</Card.Title>
                </Card.Header>
                <Card.Footer>
                  <Button>
